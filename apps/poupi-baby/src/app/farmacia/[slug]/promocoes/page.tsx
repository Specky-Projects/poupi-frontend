...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data?.marketplace) return { title: 'Promocoes de farmacia | Poupi', robots: { index: false } };
  const weak = Number(data.total ?? 0) < MIN_PRODUCTS;
  const url = `${SITE_URL}/farmacia/${slug}/promocoes`;
  const title = `Promocoes ${data.marketplace.name} | Poupi`;
  const description = `Promocoes e menores precos da ${data.marketplace.name} monitorados pelo Poupi.`;
  return {
    title,
    description,
...
export default async function Page({ params }: Props) {
  ...
  return (
    <ProgrammaticListingPage
      title={`Promocoes ${data.marketplace.name}`}
      description={`${data.total} ofertas monitoradas com revalidacao rapida para capturar mudancas de preco.`}
      breadcrumb={[{ label: 'Poupi', href: '/' }, { label: data.marketplace.name, href: `/farmacia/${slug}` }, { label: 'Promocoes' }]}
